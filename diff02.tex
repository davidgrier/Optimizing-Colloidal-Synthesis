...
%DIF ADD optimizing02.tex   Sat Feb 16 07:06:35 2019
...
\cite{vanderwel17},
which \DIFdelbegin \DIFdel{relies on }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend conventional techniques to systematically
characterize the emulsion polymerization protocol for TPM spheres.
\DIFaddbegin \DIFadd{Our holographic measurements
independently confirm the observation \mbox{%DIFAUXCMD
\cite{vanderwel17}
}\hspace{0pt}%DIFAUXCMD
that smaller particles form at higher pH.
They also reveal that more vigorous mixing yields larger
...
colloidal spheres of various compositions \mbox{%DIFAUXCMD
...
.
}\DIFaddend The solid curve in Fig.~\ref{fig:flow_prof}(b) is a fit to a
...
rather than reflecting the properties of just one}\DIFaddend .
...
}\DIFdelend \DIFaddbegin \DIFadd{, and are presented
...
the average of each oval's minor and major axes provides an estimate for the associated sphere's diameter \mbox{%DIFAUXCMD
\cite{klein2011traceable,mazzoli12}}\hspace{0pt}%DIFAUXCMD
..
...
\DIFdelbegin \DIFdel{Each sample was }\DIFdelend \DIFaddbegin \DIFadd{The initial number density of particles in each
sample is obtained with the xSight by counting
the particles in \mbox{%DIFAUXCMD
\SI{2}{\micro\liter} }\hspace{0pt}%DIFAUXCMD
of fluid.
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=0.5\textwidth]{sizedistributions03}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
(red).
  ...
  %DIFAUXCMD
%DIFDELCMD < \label{fig:sizedistribution}
...
\DIFaddbegin \DIFadd{The meniscus of the driven vortex ranges in depth
from \mbox{%DIFAUXCMD
...
\begin{table}[!h]
\centering
\caption{\DIFaddFL{Median size and standard deviation of TPM spheres
  measured by SEM analysis 
  and holographically for each of the four stir rates.}}
\begin{tabular}{rrrrr}
\hline
\hline
\DIFaddFL{$\Omega~[\si{\per\minute}]$ }& \DIFaddFL{\num{500} }& \DIFaddFL{\num{700}}& \DIFaddFL{\num{900} }& \DIFaddFL{\num{1100} }\\
\hline
\DIFaddFL{$\avg{d_p}_{\text{SEM}}~[\si{\um}]$ }& \DIFaddFL{1.45 }& \DIFaddFL{1.56 }& \DIFaddFL{1.92 }& \DIFaddFL{1.75 }\\ 
\DIFaddFL{$\sigma_{d_p, \text{SEM}}~[\si{\um}]$ }& \DIFaddFL{0.12 }& \DIFaddFL{0.13 }& \DIFaddFL{0.28 }& \DIFaddFL{0.32 }\\ 
\DIFaddFL{$\avg{d_p}_{\text{HPC}}~[\si{\um}]$ }& \DIFaddFL{1.43 }& \DIFaddFL{1.54 }& \DIFaddFL{1.88 }& \DIFaddFL{1.73 }\\ 
\DIFaddFL{$\sigma_{d_p, \text{HPC}}~[\si{\um}]$ }& \DIFaddFL{0.04 }& \DIFaddFL{0.05 }& \DIFaddFL{0.06 }& \DIFaddFL{0.08 }\\ 
\hline \hline
\end{tabular}
\label{table:sem_data}
\end{table}

\DIFaddend Size distributions obtained holographically are consistent with results of SEM analysis on the same samples, including
the images in Fig.~\ref{fig:semimages}.
\DIFdelbegin \DIFdel{The mean sphere diameter is estimated by analyzing \num{50} spheres' images using ImageJ \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{These results
}\DIFaddend also are plotted in Fig.~\ref{fig:sizedistribution}
\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and reported in Table~\ref{table:sem_data}.
...
\DIFdelFL{Median size and standard deviation of TPM spheres
  ...
  and holographically for each of the four stir rates.}}
%DIFAUXCMD
%DIFDELCMD < \begin{tabular}{rrrrr}
%DIFDELCMD < \hline
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdelFL{$\Omega~[\si{\per\minute}]$ }%DIFDELCMD < & %%%
\DIFdelFL{\num{500} }%DIFDELCMD < & %%%
\DIFdelFL{\num{700}}%DIFDELCMD < & %%%
\DIFdelFL{\num{900} }%DIFDELCMD < & %%%
\DIFdelFL{\num{1100} }%DIFDELCMD < \\
...
%DIFDELCMD < \label{table:sem_data}
%DIFDELCMD < \end{table}
%DIFDELCMD < 
